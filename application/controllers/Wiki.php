...
class Wiki extends MY_Controller
{
    public function __construct()
	{
		...
		// **PERUBAHAN**: Memuat library paginasi
	...
	public function index()
	{
		...
		$config['base_url'] = site_url('wiki/index');
		$config['total_rows'] = $this->wiki_model->count_articles($search);
		$config['per_page'] = 15; // Jumlah artikel per halaman
		...
		// Menentukan halaman saat ini dan offset
		...
		// **PERUBAHAN**: Mengirim nomor awal untuk penomoran baris
		$data['start_no'] = $offset;

		$this->render_page('wiki/index_view', $data, 'Pusat Pengetahuan');
	}

	public function article($id)
	{
		$data['article'] = $this->wiki_model->get_article_by_id($id);
		if (!$data['article']) {
            show_404();
        }

		$data['revisions'] = $this->wiki_model->get_article_revisions($id);

		$this->render_page('wiki/article_view', $data, $data['article']->title);
	}
}
